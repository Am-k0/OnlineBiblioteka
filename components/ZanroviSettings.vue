...
const headers = ref([
  ...
  { title: 'Naziv Žanra', key: 'name', value: 'name', align: 'start', sortable: true },
...
const fetchGenres = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${apiBaseUrl}/api/genres`, {
      headers: {
    ...
    // Prilagođavanje na strukturu odgovora iz PHP-a
    const apiData = response.data.genres;
    zanrovi.value = apiData.data;
    totalItems.value = apiData.total;
    
  } catch (err) {
    setError('Greška pri učitavanju žanrova: ' + (err.response?.data?.message || err.message || 'Nepoznata greška'));
...
const handleEdit = (item) => {
  console.log('Uređivanje žanra:', item);
};

const handleDelete = async (item) => {
  try {
    if (!confirm(`Da li ste sigurni da želite da obrišete žanr "${item.name}"?`)) return;
    loading.value = true;
    const token = localStorage.getItem('auth_token');
    await axios.delete(`${apiBaseUrl}/api/genres/${item.id}/destroy`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    await fetchGenres();
  } catch (err) {
    setError('Greška pri brisanju žanra: ' + (err.response?.data?.message || err.message || 'Nepoznata greška'));
  } finally {
    loading.value = false;
  }
};
...
watch(itemsPerPage, () => { fetchGenres(); });
