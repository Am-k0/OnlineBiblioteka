<template>
  <div class="pa-4">
    <v-btn color="primary" class="mb-4" @click="handleNewCategory">
    ...
    <v-data-table
      ...
      :items="kategorije"
      :loading="loading"
      loading-text="Učitavam kategorije..."
      item-value="id"
...
const handleNewCategory = () => {
  // Povezivanje dugmeta sa formom za kreiranje kategorije
  router.push('/category');
};

const handleEdit = (item) => {
  // Dodajte logiku za uređivanje, npr. router.push(`/settings/category/${item.id}`);
  console.log('Uređivanje kategorije:', item);
};

const handleDelete = async (item) => {
  try {
    if (!confirm(`Da li ste sigurni da želite da obrišete kategoriju "${item.name}"?`)) return;
    loading.value = true;
    const token = localStorage.getItem('auth_token');
    await axios.delete(`${apiBaseUrl}/api/categories/${item.id}/destroy`, {
